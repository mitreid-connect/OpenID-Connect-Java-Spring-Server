...
public class DefaultOAuth2ProviderTokenService implements OAuth2TokenEntityService {
      ...
      if (request.getExtensions().containsKey(CODE_CHALLENGE)) {
        ...
        if (alg.equals(PKCEAlgorithm.plain)) {
          ...
          try {
            ...
            String hash = Base64URL
              .encode(digest.digest(verifier.getBytes(StandardCharsets.US_ASCII))).toString();
  ...
  public OAuth2AccessTokenEntity refreshAccessToken(String refreshTokenValue,
    ...
    // get the stored scopes from the authentication holder's authorization request; these are the
    ...
    if (scope != null && !scope.isEmpty()) {
      ...
      // otherwise inherit the scope of the refresh token (if it's there -- this can return a null
      // scope set)
   ...
   * 
   * @see
   * org.mitre.oauth2.service.OAuth2TokenEntityService#getAccessTokensForClient(org.mitre.oauth2.
   * model.ClientDetailsEntity)
   */
  @Override
   ...
   * 
   * @see
   * org.mitre.oauth2.service.OAuth2TokenEntityService#getRefreshTokensForClient(org.mitre.oauth2.
   * model.ClientDetailsEntity)
   */
  @Override
  ...
  public void clearExpiredTokens() {
    ...
    new AbstractPageOperationTemplate<AuthenticationHolderEntity>(
      ...
      public void doOperation(AuthenticationHolderEntity item) {
        authenticationHolderService.remove(item);
      }
    }.execute();
  }
