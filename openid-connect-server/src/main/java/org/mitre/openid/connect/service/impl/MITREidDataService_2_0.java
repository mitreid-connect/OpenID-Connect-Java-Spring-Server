...
/**
 *
 * Data service to import and export MITREid 2.0 configuration.
 *
 * @author jricher
 * @author arielak
 */
...
public class MITREidDataService_2_0 extends MITREidDataServiceSupport implements MITREidDataService {
	 ...
	 */
	private static final Logger logger = LoggerFactory.getLogger(MITREidDataService_2_0.class);
	@Autowired
	...
	private static final String THIS_VERSION = MITREID_CONNECT_2_0;
	...
	public void exportData(JsonWriter writer) throws IOException {
		...
		writer.endObject(); // end mitreid-connect-2.0
	}
	...
	public void importData(JsonReader reader) throws IOException {
		...
		logger.info("Reading configuration for 2.0");
				...
				switch (reader.peek()) {
					...
					case NAME:
						...
						if (reader.peek() == JsonToken.NULL) {
							...
							authHolderId = reader.nextString();
						} else {
							logger.debug("Found unexpected entry");
							reader.skipValue();
						}
				...
				switch (reader.peek()) {
					...
					case NAME:
						...
						if (reader.peek() == JsonToken.NULL) {
							...
							refreshTokenId = reader.nextString();
