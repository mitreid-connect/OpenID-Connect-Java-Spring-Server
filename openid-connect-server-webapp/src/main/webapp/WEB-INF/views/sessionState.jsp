...
<html>
<head>
    <title>${config.topbarTitle} - Session State OP IFrame</title>
        ...
        (function (config) {
            ...
            function checkSessionWithBackend(sid, targetWindow, eventOrigin) {
                ...
                if (current - lastCheck >= config.revalidate * 1000) {
                    ...
                    oReq.addEventListener("load", function () {
                        var result = (oReq.responseText) ? JSON.parse(oReq.responseText) : "";
                        targetWindow.postMessage(result, eventOrigin);
                    });
                    oReq.open("GET", config.validationUrl);
                    oReq.setRequestHeader("X-Session-State", sid);
                    oReq.send();
                } else {
                    targetWindow.postMessage("unchanged", eventOrigin);
                }
            }
            ...
            function receiveMessage(event) {
                ...
                try {
                    ...
                    // get the op session state cookie value
                    var opss = getCookie(config.cookieName);
                    ...
                    var hashInput = client_id + " " + event.origin + " " + opss + " " + salt;
                    ...
                    if (expectedHash === receivedHash) {
                        // Check session ID with the MITREid server backend to prevent
                        // false "unchanged" on expired session ID when cookie is still
                        // present
                        checkSessionWithBackend(opss, event.source, event.origin);
                    } else {
                        event.source.postMessage("changed", event.origin);
                    }

                } catch (e) {
                    logMessage("Check session error: " + e);
                    event.source.postMessage("error", event.origin);
                }
            }
